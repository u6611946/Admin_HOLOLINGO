...
const DEFAULT_TOKEN_FLOOR = 10;
const DEFAULT_FREE_TOP_UP = 75;

export default function TokenBattlePage() {
  ...
  const [tokenFloor, setTokenFloor] = useState(String(DEFAULT_TOKEN_FLOOR));
  ...
  useEffect(() => {
    ...
    return onSnapshot(doc(db, 'config', 'token_battle_settings'), (snapshot) => {
      const data = snapshot.data();
      setTokenFloor(String(data?.token_floor ?? DEFAULT_TOKEN_FLOOR));
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      await setDoc(
        doc(db, 'config', 'token_battle_settings'),
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '900px', color: theme.text }}>
      <div style={{ marginBottom: '24px' }}>
        <div style={{ color: theme.textStrong, fontSize: '20px', fontWeight: 700 }}>Token Battle</div>
        <div style={{ color: theme.textMuted, fontSize: '12px', marginTop: '3px' }}>
          1v1 wager battles — live activity and token economy settings
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '14px', marginBottom: '28px' }}>
        {[
          ...
          { label: 'Tokens wagered (resolved)', value: totalWagered.toLocaleString(), color: '#f59e0b' },
      ...
      <div style={{ background: theme.bgCard, border: `1px solid ${theme.border}`, borderRadius: '14px', padding: '18px', marginBottom: '28px' }}>
        <div style={{ color: theme.textStrong, fontSize: '14px', fontWeight: 700, marginBottom: '4px' }}>Token economy settings</div>
        <div style={{ color: theme.textMuted, fontSize: '11px', marginBottom: '16px' }}>
          Read by the app at battle time — token_floor is baked into each new battle so a losing streak can never fully wipe someone out.
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px', opacity: settingsLoaded ? 1 : 0.5 }}>
          <div>
            ...
            <input
              ...
              onChange={(event) => setTokenFloor(event.target.value)}
            />
          </div>
          <div>
            <div style={{ color: theme.textMuted, fontSize: '10px', textTransform: 'uppercase', letterSpacing: '.06em', marginBottom: '6px' }}>
